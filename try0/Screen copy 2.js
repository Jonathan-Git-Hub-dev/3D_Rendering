...
const height_pixels = 100
            ...
            //let points1 = [[0,3,6],[0,3,3],[0,6,3],[0,6,6]];
            //let points2 = [[3,3,6],[3,3,3],[3,6,3],[3,6,6]];
            let points1 = [[0,0,6],[0,4,6],[4,0,6],[4,4,6]];
            let points2 = [[0,0,10],[0,4,10],[4,0,10],[4,4,10]];
            //let points1 = [[5,8,2]]
            //let points1 = [[5,5,1],[5,5,4],[8,5,4],[5,8,4],[5,8,1],[8,5,1],    [10,5,1],[10,5,4],[13,5,4],[10,8,4],[10,8,1],[13,5,1]];
            //let points2 = []//[8,8,4],[8,8,1],  [13,8,4],[13,8,1]];
            //let points1 = [[3.5,3.5,4],[3.5,6.5,4],[6.5,3.5,1],[6.5,6.5,1]];
            //let points2 = [[6.5,8,4],[8,6.5,4],[8,8,1]]//,[,,1]];
...
export default function Screen()
{
    ...
    {
        if(screen_origin[0]-origin[0] != 0)//need to be facing x so ratio works
        {
            ...
            //let angle = Math.atan((screen_origin[1]-origin[1])/(screen_origin[0]-origin[0]));
            //console.log((screen_origin[1]-origin[1])+" "+(screen_origin[0]-origin[0]))
            //console.log(distance)
            //console.log(angle)
            //console.log("z rat ::::: " + z_to_x_ratio + " " + )
            let x = Math.sqrt(distance/(1+y_to_x_ratio**2+z_to_x_ratio**2))//5,8,2 (0.5184758473652126, 0.82956135578, 0.20739033894)
            console.log("dist " + distance)
            console.log("xxxx: " + x)
            
            //let x= Math.sqrt(distance**2/(1+Math.tan(angle)**2))
            //let y = x*Math.tan(angle)
            //console.log("best x so far: " + x)

            //i think this is always relative to 0,0,0 so add extra
        ...
        else if(screen_origin[1]-origin[1] != 0)//need to be facing x so ratio works
        {
            ...
            //console.log(angle)
            //console.log("z rat ::::: " + z_to_x_ratio + " " + )
            let y = Math.sqrt(distance/(1+x_to_y_ratio**2+z_to_y_ratio**2))//5,8,2 (0.5184758473652126, 0.82956135578, 0.20739033894)
            //console.log("dist " + distance)
            //console.log("xxxx: " + x)
        ...
        else
        {
            ...
            //console.log(angle)
            //console.log("z rat ::::: " + z_to_x_ratio + " " + )
            let z = Math.sqrt(distance/(1+x_to_z_ratio**2+y_to_z_ratio**2))//5,8,2 (0.5184758473652126, 0.82956135578, 0.20739033894)
            //console.log("dist " + distance)
            //console.log("xxxx: " + x)
    ...
    function dist(point, colour, origin, screen_origin)
    {
        ...
        let tempy = second_point_3d(origin, screen_origin, point);
        let n_s = [origin[0]+ tempy[0],origin[1]+ tempy[1],origin[2]+ tempy[2]]//new center]
        
        //rounding error can occur see 5 - 5.000000000000001  
        n_s=[n_s[0].toFixed(5),n_s[1].toFixed(5),n_s[2].toFixed(5)]
        ...
        let z_len;
        //better angels
        if(screen_origin[2]-origin[2]!=0)
        {
            //console.log("doing z stuff");
            //not 100% sure but probs sin
            if(screen_origin[1]-origin[1] == 0)//flat with x
            {
                x_len_down = Math.sin(Math.atan((screen_origin[0]-origin[0])/(screen_origin[2]-origin[2])))
            }
            if(screen_origin[0]-origin[0] == 0)//flat with y
            {
                //y_len_down = Math.sin(Math.atan((screen_origin[1]-origin[1])/(screen_origin[2]-origin[2])))
                //new one
                y_len_down = Math.sin(Math.atan((screen_origin[2]-origin[2])/(screen_origin[1]-origin[1])))
                console.log("atan " + Math.atan((screen_origin[1]-origin[1])/(screen_origin[2]-origin[2])))
            ...
            console.log("z stuff done: " + x_len_down + " " + y_len_down)
        }
        ...
        {//z
            //z_len=0
            console.log("THIS NEEDS TO BE DONE LATER");
        ...
        else
        {
            ...
            //I think they either x or y should be negative this will be worng so fix later
            if(screen_origin[0] > 0)
            {
                y_angle*=-1
            }
            else
            ...
            if(screen_origin[2]- origin[2] != 1)
            {
                //when facing up or down z y and z contribute to depth
        ...
        //console.log(point[0])
        //console.log(n_s[0])
        //console.log(point[0]-n_s[0])
        let x_dist = (point[0]-n_s[0]) * x_len 
        let y_dist = (point[1]-n_s[1]) * y_len
        let x_dist_down = (point[0]-n_s[0]) * x_len_down
        let y_dist_down = (point[1]-n_s[1]) * y_len_down
        console.log("this is the new stuff " + x_dist_down + " " +y_dist_down)
        //console.log("z_dist " + z_dist);
        console.log("x_dist "+(x_dist/x_len)+ " , "+ x_dist);
        console.log((point[1]-n_s[1]))
        console.log(y_len)
        ...
        let h = ((z_dist+x_dist_down+y_dist_down+height/2)/height)*height_pixels
    ...
    return(
        ...
        <button onClick={async ()=>{
            ...
            //{
                ...
                //let screen_origin = [0, 0.70710678118 ,0.70710678118]
                let screen_origin = [0,0.00447,0.99999]
                let origin = [0,0,0];//xyz
                //figure out angle
